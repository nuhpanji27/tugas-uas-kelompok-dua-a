...
>
	...
	>
		...
		<div class="sidebar-brand-text mx-3">Desa Sirnaraja</div>
	</a>
	...
	<?php
    $role_id = $this->session->userdata('role_id'); $querymenu = "SELECT
	`user_menu`.`id`, `name` FROM `user_menu` JOIN `user_access_menu` ON
	`user_menu`.`id` = `user_access_menu`.`menu_id` WHERE
	`user_access_menu`.`role_id` = $role_id ORDER BY `user_access_menu`.`menu_id`
	ASC "; $menu = $this->db->query($querymenu)->result_array(); 
	//var_dump($menu); // die; ?>
